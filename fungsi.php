...
// mencari ID kriteria
// berdasarkan urutan ke berapa (C1, C2, C3 ,C4 ,C5)
function getKriteriaID($no_urut) {
    ...
    // Cek apakah array $listID memiliki kunci yang sesuai
    if (isset($listID[$no_urut])) {
        return $listID[$no_urut];
    } else {
        return null; // Atau lakukan penanganan sesuai kebutuhan Anda
    }
}
...
function getNilaiPerbandinganKriteria($kriteria1, $kriteria2) {
    ...
    // Validasi nilai input untuk menghindari SQL injection
    $id_kriteria1 = (int)$kriteria1;
    $id_kriteria2 = (int)$kriteria2;

    $query  = "SELECT nilai FROM perbandingan_kriteria WHERE kriteria1 = $id_kriteria1 AND kriteria2 = $id_kriteria2";
    $result = mysqli_query($koneksi, $query);

    if (!$result) {
        echo "Error: " . mysqli_error($koneksi); // Tampilkan pesan kesalahan SQL
        exit();
    }
                    ...
                    for ($y = 0; $y < $n; $y++) {
                        ...
                        if ($x == $y) {
                            ...
                            if ($x < $y) {
                                ...
                                ?>
                                <input type="number" step="0.01" name="bobot<?php echo $x . '_' . $y; ?>" value="<?php echo $nilai; ?>" required>
                                <?php
                            } else {
                                echo "<input type='text' disabled value='" . (1 / getNilaiPerbandinganKriteria($y, $x)) . "'>";
                            }
                        }
